...
Escribe la fecha actual (**NOTE-01**): 04 de junio de 2021
...
```{r}
NOMBRE_COMPLETO = "Paola Temores"
DIGITOS_MATRICULA = 01376178
```
...
- ¿Cuál es la interpretación de la regla `{} => {diapers}` en la que se aprecia la ausencia del LHS? (**NOTE-19**):No hay relación entre la compra de pañales con la compra de otro artículo
...
- ¿Por qué no aparece listada la regla `{beer} => {nuts}`? (**NOTE-22**):Porque las métricas de confidence y support excluyen a la regla 
...
- ¿Cuál es el propósito de `session id`? (**NOTE-23**):A partir del identificador único por usuario, es posible proporcionar recomendaciones personalizadas para cada usuario dependiendo de sus preferencias 
- ¿Cuál es el propósito de `track id`? (**NOTE-24**): A partir del identificador único por canción es posible medir tendencias relacionadas a género o artistas relacionados
- ¿Consideras que las otras columnas del dataset son aplicables para análisis por medio de reglas de asociación, justifica tu respuesta? (**NOTE-25**):Sí, dado que es posible identficar tendencias en estos otros parámetros al formar relaciones con otras columnas, algo que puede llevarse a cabo usando las reglas de asociación
...
- ¿Cuál es el ID y frecuencia absoluta de la sesión más frecuente? (**NOTE-26**):t_bacf06d3-9185-4183-84ea-ff0db51475ce con frecuencia de 877 
- ¿Cuántos items tiene el itemset con mayor número de ocurrencias en la lista element length distribution? (**NOTE-27**):20 con 1555 ocurencias 
- En el contexto de este dataset con información de sesiones de streaming, ¿cuál es la interpretación de la media aritmética? (**NOTE-28**):Los usuarios ingresan en promedio 14.57 veces por sesión 
...
- ¿Qué factores están asociados con que el algoritmo `apriori` con parámetros por omisión no haya producido ninguna regla de asociación, justifica tu respuesta? (**NOTE-29**): Porque ninguno de los items cumple con los parámetros de support y confidence que se le asignaron.
- ¿Qué riesgos identificas con la asignación de valores muy bajos en los parámetros de  `support` y `confidence` al invocar el algoritmo `apriori`? Un mayor tiempo de ejecución y menor precisión al no incorporar valores potencialmente importantes
